...
def train_cnn_with_warmup(data_path, tfidf_vectorizer_path, keras_vectorizer_path, label_binarizer_path, model_path):
    ...
    for i, cluster_size in enumerate([16, 256, 4096, Y_vec.shape[1]]):
        ...
        if cluster_size == Y_vec.shape[1]:
            print("   no need to cluster")
            M = Y_vec
        else:
            print("   clustering")
            start = time.time()
            kmeans = MiniBatchKMeans(n_clusters=cluster_size, max_iter=10, n_init=3, verbose=1)
            K = kmeans.fit_predict(L_vec).reshape(-1, 1)
            clustering_time = time.time() - start
            print(f"   time elapsed: {clustering_time}s")
        ...
        if i == 0:
            ...
            if implementation == 1:
                ...
                cnn = CNNClassifier(multilabel=True, sparse_y=True, batch_size=256, learning_rate=1e-4, l2=l2, nb_epochs=1, hidden_size=200, dense_size=512)
                cnn.build_model(vectorizer.sequence_length, vectorizer.vocab_size, nb_outputs=M.shape[1], steps_per_epoch=X_vec.shape[0] / 256)
                for i, weights in enumerate(cnn_weights[:-1]):
                    cnn.model.layers[i].set_weights(weights)

        cnn.fit(X_vec, M)
        cnn_weights = cnn.model.get_weights()
