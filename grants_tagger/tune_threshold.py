...
from sklearn.metrics import f1_score, multilabel_confusion_matrix as sk_multilabel_confusion_matrix
...
logger.setLevel(os.environ.get("LOGGING_LEVEL", logging.INFO)) # this should inherit from root
...
def argmaxf1(Y_test, Y_pred_proba, optimal_thresholds, label_i, cm, nb_thresholds, iterations):
    ...
    for candidate_threshold in candidate_thresholds:
        ...
        if new_f1 > max_f1:
            ...
            logger.debug(f"Label: {label_i:4d} - f1: {max_f1:.6f} - Th: {optimal_threshold:.3f}")
...
def optimise_threshold(Y_test, Y_pred_proba, nb_thresholds=None, init_threshold=None):
    ...
    while updated:
        ...
        argmaxf1_partial = partial(argmaxf1, Y_test, Y_pred_proba, optimal_thresholds, cm=cm, nb_thresholds=nb_thresholds, iterations=iterations)
        ...
        print(f"Iteration {iterations} - f1 {optimal_f1:.3f} - time spent {time_spent}s")
...
def tune_threshold(approach, data_path, model_path, label_binarizer_path, thresholds_path, val_size: float=0.8,
        nb_thresholds: int=None, init_threshold=None, split_data=False):
